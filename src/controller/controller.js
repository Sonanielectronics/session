...
class class1 {
  ...
  static i = async (req, res) => {
    ...
    for (var i = 0; i < OriginalData.length; i++) {
      ...
      if (OriginalData[i].TotleGameTime > 0) {
        OriginalData[i].OfficialTotleGameTime =
          OriginalData[i].OfficialTotleGameTime - 1;
        OriginalData[i].TotleGameTime = OriginalData[i].TotleGameTime - 1;
        OriginalData[i].save();

        // Bug Solve: Data Update LastWinningNumber Variable
        if (OriginalData[i].TotleGameTime <= 15) {
          function indexOfMinValue(array) {
            if (array.length === 0) {
              return -1; // Return -1 for an empty array
            }
          ...
          await Todo2.updateOne({}, { LastWinningNumber: WinningNumberIndex });
        }
      } else {
  ...
  static delete = async (req, res) => {
    try {
      ...
      await Todo2.find({}).deleteMany();
      await Todo3.find({}).deleteMany();
      await Todo4.find({}).deleteMany();
